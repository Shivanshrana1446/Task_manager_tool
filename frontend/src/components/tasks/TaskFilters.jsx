...
const TaskFilters = ({ filters, onChange, currentUserId }) => {
  ...
  return (
    <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <label className="flex items-center gap-1.5 whitespace-nowrap text-sm text-foreground/60">
          <input
            type="checkbox"
            checked={filters.assignee === currentUserId}
            onChange={(event) => update({ assignee: event.target.checked ? currentUserId : '' })}
